...
go_tool "submit_supply_chain_map" {
  description = "Write the evidence-linked reference supply chain, continuity-risk assessment targets, and separate public-company mapping targets. `nodes.kind` allowed values: `product`, `component`, `material`, `process`, `equipment`, `qualification`, `service`, `system`. `edges.relation` allowed values: `contains`, `supplies`, `transformed_into`, `assembled_into`, `processed_by`, `tested_by`, `qualified_by`, `used_by`. `company_mapping_targets` may reference only supplier-addressable `component`, `material`, `process`, `equipment`, or `service` nodes. This tool does not score, rank, or declare chokepoints."
  source = <<-GO
    import("context";"encoding/json";"fmt";"os";"path/filepath";"strings")
...
go_tool "submit_company_priorities" {
  description = "Write a company research-priority list for one selected company-mapping target from the authoritative supply-chain artifact. Every public-company record requires exact company name, non-empty ticker, and market. `companies.role` allowed values: `existing_supplier`, `qualified_alternative`, `capability_match`, `related_product_only`, `unverified`. `companies.research_priority` allowed values: `A`, `B`, `C`, `do_not_research`. A/B/C are follow-up research priorities, never investment ratings; A requires confirmed exact-node evidence in `relationship_claim_ids` for an existing supplier or qualified alternative, or in `capability_claim_ids` for a capability match. `companies.economic_exposure.*.evidence_directness` allowed values: `none`, `confirmed`, `reported`, `inferred`. `companies.economic_exposure.customer_validation.status` allowed values: `unknown`, `evaluation`, `qualified`, `ordered`, `delivering`, `production_use`. `companies.economic_exposure.revenue_materiality.status` allowed values: `unknown`, `exposure_unquantified`, `quantified_immaterial`, `quantified_material`. `companies.economic_exposure.bottleneck_capture.status` allowed values: `unknown`, `none`, `plausible`, `demonstrated`. `companies.economic_exposure.commercialization_timing.status` allowed values: `unknown`, `current`, `within_12_months`, `beyond_12_months`. Every economic-exposure dimension has `status`, `evidence_directness`, and `claim_ids`; unknown requires none and no claim IDs, while every non-unknown status requires claim IDs whose claim status matches evidence_directness. `companies.exposure_signals.scope` allowed values: `company`, `segment`, `modality`, `target_branch`, `named_program`. Each optional signal records subject, metric, value, as_of, claim_ids, and `companies.exposure_signals.evidence_directness` allowed values: `confirmed`, `reported`, `inferred`, matching every referenced claim."
  source = <<-GO
    import("context";"encoding/json";"fmt";"os";"path/filepath";"strings")
    type ExposureAssessment struct { Status string `json:"status"`; EvidenceDirectness string `json:"evidence_directness"`; ClaimIDs []string `json:"claim_ids"` }
    type EconomicExposure struct { CustomerValidation ExposureAssessment `json:"customer_validation"`; RevenueMateriality ExposureAssessment `json:"revenue_materiality"`; BottleneckCapture ExposureAssessment `json:"bottleneck_capture"`; CommercializationTiming ExposureAssessment `json:"commercialization_timing"` }
    type ExposureSignal struct { Scope string `json:"scope"`;Subject string `json:"subject"`;Metric string `json:"metric"`;Value string `json:"value"`;AsOf string `json:"as_of"`;EvidenceDirectness string `json:"evidence_directness"`;ClaimIDs []string `json:"claim_ids"` }
    type Company struct { Company string `json:"company"`; Ticker string `json:"ticker"`; Market string `json:"market"`; Role string `json:"role"`; ResearchPriority string `json:"research_priority"`; RelationshipClaimIDs []string `json:"relationship_claim_ids"`; CapabilityClaimIDs []string `json:"capability_claim_ids"`; EconomicExposure EconomicExposure `json:"economic_exposure"`; ExposureSignals []ExposureSignal `json:"exposure_signals"`; WhyResearch string `json:"why_research"`; LargestUnknown string `json:"largest_unknown"`; NextCheck string `json:"next_check"` }
    type Input struct { WorkspaceDir string `json:"workspace_dir"`; ArtifactID string `json:"artifact_id"`; ArtifactPath string `json:"_r42_artifact_path"`; SupplyChainPath string `json:"supply_chain_path"`; TargetNodeID string `json:"target_node_id"`; ClaimPaths []string `json:"claim_paths"`; Companies []Company `json:"companies"`; Conclusion string `json:"conclusion"` }
    type Output string
    ...
    type priorityNode struct { ID string `json:"id"`;Name string `json:"name"`;Kind string `json:"kind"` }
    type priorityMappingTarget struct { NodeID string `json:"node_id"`;NodeName string `json:"node_name"` }
    type prioritySupplyChain struct { ArtifactKind string `json:"artifact_kind"`;Nodes []priorityNode `json:"nodes"`;CompanyMappingTargets []priorityMappingTarget `json:"company_mapping_targets"` }
    type priorityDocument struct { NodeID string `json:"node_id"`;NodeName string `json:"node_name"`;NodeKind string `json:"node_kind"`;Companies []Company `json:"companies"`;Conclusion string `json:"conclusion"` }
    func Invoke(_ context.Context,input Input)(ToolResponse[Output],error){cwd,err:=os.Getwd();if err!=nil{return ToolResponse[Output]{},err};issues:=[]Issue{};workspace:=priorityClean(input.WorkspaceDir);root:=priorityBlocksRoot(cwd);if workspace==""||root==""||!priorityWithin(workspace,root){issues=append(issues,priorityIssue("workspace_dir","workspace_dir","must be an absolute directory inside the current run's blocks directory"))};artifact:=priorityClean(input.ArtifactPath);if artifact==""||workspace==""||filepath.Base(artifact)!="company-priorities.json"||!priorityWithin(artifact,workspace){issues=append(issues,priorityIssue("artifact_path","artifact_path","must end in company-priorities.json under workspace_dir"))};supplyPath:=priorityClean(input.SupplyChainPath);supplyReady:=supplyPath!=""&&root!=""&&filepath.Base(supplyPath)=="supply-chain.json"&&priorityWithin(supplyPath,root);if !supplyReady{issues=append(issues,priorityIssue("supply_chain_path","supply_chain_path","must name an existing supply-chain.json inside the current run's blocks directory"))}
      claims,taskClaimCards,hasTaskClaims,loadErr:=priorityClaims(input.ClaimPaths,root,workspace);if loadErr!=nil{return ToolResponse[Output]{},fmt.Errorf("load claim cards: %w",loadErr)};if !hasTaskClaims{issues=append(issues,priorityIssue("claim_paths","claim_paths","claim_paths must include the current task workspace claims.json"))};var supply prioritySupplyChain;if supplyReady{payload,readErr:=os.ReadFile(supplyPath);if readErr!=nil{issues=append(issues,priorityIssue("supply_chain_path","supply_chain_path","supply-chain.json must exist and be readable"))}else if readErr=json.Unmarshal(payload,&supply);readErr!=nil{issues=append(issues,priorityIssue("supply_chain_path","supply_chain_path","supply-chain.json must contain valid JSON"))}else if supply.ArtifactKind!="r42_supply_chain"{issues=append(issues,priorityIssue("supply_chain_path","supply_chain_path","artifact_kind must be r42_supply_chain"))}}
      nodes:=map[string]priorityNode{};for _,candidate:=range supply.Nodes{nodes[candidate.ID]=candidate};selected:=false;var node priorityNode;for _,target:=range supply.CompanyMappingTargets{if target.NodeID==input.TargetNodeID{selected=true;node=nodes[target.NodeID];if node.ID==""||target.NodeName!=node.Name{issues=append(issues,priorityIssue("target_node_id","target_node_id","selected mapping target must resolve to an exact supply-chain node"))};break}};if strings.TrimSpace(input.TargetNodeID)==""||!selected{issues=append(issues,priorityIssue("target_node_id","target_node_id","must reference a selected company_mapping_target in supply-chain.json"))}
      if strings.TrimSpace(input.Conclusion)==""{issues=append(issues,priorityIssue("conclusion","conclusion","conclusion must not be empty"))};roles:=map[string]bool{"existing_supplier":true,"qualified_alternative":true,"capability_match":true,"related_product_only":true,"unverified":true};levels:=map[string]bool{"A":true,"B":true,"C":true,"do_not_research":true}
      for index,company:=range input.Companies{path:=fmt.Sprintf("companies[%d]",index);for field,value:=range map[string]string{"company":company.Company,"ticker":company.Ticker,"market":company.Market,"why_research":company.WhyResearch,"largest_unknown":company.LargestUnknown,"next_check":company.NextCheck}{if strings.TrimSpace(value)==""{issues=append(issues,priorityIssue("required",path+"."+field,field+" must not be empty"))}};if !roles[company.Role]{issues=append(issues,priorityIssue("role",path+".role","unsupported company role"))};if !levels[company.ResearchPriority]{issues=append(issues,priorityIssue("research_priority",path+".research_priority","research_priority must be A, B, C, or do_not_research"))};issues=append(issues,priorityClaimReferenceIssues(path+".relationship_claim_ids",company.RelationshipClaimIDs,claims)...);issues=append(issues,priorityClaimReferenceIssues(path+".capability_claim_ids",company.CapabilityClaimIDs,claims)...);issues=append(issues,priorityExposureIssues(path,company.EconomicExposure,claims)...);issues=append(issues,prioritySignalIssues(path,company.ExposureSignals,claims)...)
        if company.ResearchPriority=="A"{validRelationship:=(company.Role=="existing_supplier"||company.Role=="qualified_alternative")&&priorityConfirmedClaims(company.RelationshipClaimIDs,claims);validCapability:=company.Role=="capability_match"&&priorityConfirmedClaims(company.CapabilityClaimIDs,claims);if !validRelationship&&!validCapability{issues=append(issues,priorityIssue("research_priority",path+".research_priority","A requires confirmed exact-node relationship or capability evidence matching the declared role"))}}
      }
      if len(issues)>0{return ToolResponse[Output]{Accepted:false,Issues:issues},nil};document:=priorityDocument{NodeID:node.ID,NodeName:node.Name,NodeKind:node.Kind,Companies:input.Companies,Conclusion:input.Conclusion};payload,err:=json.MarshalIndent(document,"","  ");if err!=nil{return ToolResponse[Output]{},err};if err=os.MkdirAll(filepath.Dir(artifact),0700);err!=nil{return ToolResponse[Output]{},err};if err=os.WriteFile(artifact,append(payload,'\n'),0600);err!=nil{return ToolResponse[Output]{},err};result,err:=json.Marshal(map[string]any{"node_id":document.NodeID,"node_name":document.NodeName,"node_kind":document.NodeKind,"companies":document.Companies,"conclusion":document.Conclusion,"claims":taskClaimCards});if err!=nil{return ToolResponse[Output]{},err};output:=Output(result);return ToolResponse[Output]{Accepted:true,Output:&output},nil}
    ...
    func priorityClaimReferenceIssues(path string,ids []string,claims map[string]string)[]Issue{issues:=[]Issue{};for _,id:=range ids{if _,ok:=claims[id];!ok{issues=append(issues,priorityIssue("claim_id",path,id+" does not exist"))}};return issues}
    func priorityConfirmedClaims(ids []string,claims map[string]string)bool{if len(ids)==0{return false};for _,id:=range ids{if claims[id]!="confirmed"{return false}};return true}
    func priorityExposureIssues(path string,exposure EconomicExposure,claims map[string]string)[]Issue{customer:=map[string]bool{"unknown":true,"evaluation":true,"qualified":true,"ordered":true,"delivering":true,"production_use":true};materiality:=map[string]bool{"unknown":true,"exposure_unquantified":true,"quantified_immaterial":true,"quantified_material":true};capture:=map[string]bool{"unknown":true,"none":true,"plausible":true,"demonstrated":true};timing:=map[string]bool{"unknown":true,"current":true,"within_12_months":true,"beyond_12_months":true};dimensions:=[]struct{name string;value ExposureAssessment;allowed map[string]bool}{{"customer_validation",exposure.CustomerValidation,customer},{"revenue_materiality",exposure.RevenueMateriality,materiality},{"bottleneck_capture",exposure.BottleneckCapture,capture},{"commercialization_timing",exposure.CommercializationTiming,timing}};issues:=[]Issue{};directness:=map[string]bool{"none":true,"confirmed":true,"reported":true,"inferred":true};for _,dimension:=range dimensions{dimensionPath:=path+".economic_exposure."+dimension.name;if !dimension.allowed[dimension.value.Status]{issues=append(issues,priorityIssue("economic_exposure",dimensionPath+".status","unsupported economic exposure status"))};if !directness[dimension.value.EvidenceDirectness]{issues=append(issues,priorityIssue("economic_exposure",dimensionPath+".evidence_directness","evidence_directness must be none, confirmed, reported, or inferred"))};if dimension.value.Status=="unknown"{if dimension.value.EvidenceDirectness!="none"||len(dimension.value.ClaimIDs)>0{issues=append(issues,priorityIssue("economic_exposure",dimensionPath,"unknown requires evidence_directness none and no claim_ids"))}}else if dimension.allowed[dimension.value.Status]&&(dimension.value.EvidenceDirectness=="none"||len(dimension.value.ClaimIDs)==0){issues=append(issues,priorityIssue("economic_exposure",dimensionPath,"a non-unknown status requires evidence_directness and claim_ids"))};for _,id:=range dimension.value.ClaimIDs{status,ok:=claims[id];if !ok{issues=append(issues,priorityIssue("claim_id",dimensionPath+".claim_ids",id+" does not exist"));continue};if dimension.value.EvidenceDirectness!="none"&&status!=dimension.value.EvidenceDirectness{issues=append(issues,priorityIssue("economic_exposure",dimensionPath+".evidence_directness","evidence_directness must match every referenced claim status"))}}};return issues}
    func prioritySignalIssues(path string,signals []ExposureSignal,claims map[string]string)[]Issue{issues:=[]Issue{};scopes:=map[string]bool{"company":true,"segment":true,"modality":true,"target_branch":true,"named_program":true};directness:=map[string]bool{"confirmed":true,"reported":true,"inferred":true};for index,signal:=range signals{signalPath:=fmt.Sprintf("%s.exposure_signals[%d]",path,index);if !scopes[signal.Scope]{issues=append(issues,priorityIssue("exposure_signal",signalPath+".scope","unsupported exposure signal scope"))};for field,value:=range map[string]string{"subject":signal.Subject,"metric":signal.Metric,"value":signal.Value,"as_of":signal.AsOf}{if strings.TrimSpace(value)==""{issues=append(issues,priorityIssue("required",signalPath+"."+field,field+" must not be empty"))}};if !directness[signal.EvidenceDirectness]{issues=append(issues,priorityIssue("exposure_signal",signalPath+".evidence_directness","evidence_directness must be confirmed, reported, or inferred"))};if len(signal.ClaimIDs)==0{issues=append(issues,priorityIssue("exposure_signal",signalPath+".claim_ids","at least one claim ID is required"))};for _,id:=range signal.ClaimIDs{status,ok:=claims[id];if !ok{issues=append(issues,priorityIssue("claim_id",signalPath+".claim_ids",id+" does not exist"));continue};if directness[signal.EvidenceDirectness]&&status!=signal.EvidenceDirectness{issues=append(issues,priorityIssue("exposure_signal",signalPath+".evidence_directness","evidence_directness must match every referenced claim status"))}}};return issues}
