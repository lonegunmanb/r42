#!/usr/bin/env python3
"""Perplexity external tools for the r42 chokepoint example."""
...
def reject(path: str, message: str) -> dict[str, Any]:
    return {
        "accepted": False,
        "issues": [{"code": "invalid_arguments", "message": message, "path": path}],
    }
...
def search(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    decoded = post_json(
        "/search",
        {"query": query, "max_results": 10, "search_context_size": "high"},
    )
    results = []
...
def extract_fetched_content(
    decoded: dict[str, Any], requested_url: str
) -> tuple[str, str, str]:
    fallback = ""
...
def fetch(arguments: dict[str, Any]) -> dict[str, Any]:
    ...
    decoded = post_json(
        "/v1/agent",
        {
            ...
            "instructions": "Use fetch_url for the provided URL. Return only fetched content.",
    ...
    digest = hashlib.sha256(fetched_url.encode("utf-8")).hexdigest()[:16]
    snapshot_path = (pathlib.Path.cwd() / f"snapshot-{digest}.md").resolve()
    snapshot = "\n".join(
        [
            f"# {title}",
            "",
