...
3. 今晨大事，结构上保留“事实、影响、观察”，正文则写成自然的中文短段落；
4. 今日主线与市场影响，以及盘前观察清单，明确触发条件、传导链、影响方向、确认信号、失效条件、期限和置信度；
5. 机构信息扫描，覆盖市场与资金面、债券利率、商品、公司公告和机构分歧；
6. 今日事件表与上一交易时段的市场信号；
...
`edition_date` 默认为 `null`；未显式设置时，示例按 `Asia/Shanghai`（UTC+8）
计算当前日期。需要补跑或重现历史版本时，可在变量文件中用 `YYYY-MM-DD` 显式覆盖。
...
阿里巴巴、美团、京东、小米、网易。它是 `var.watchlist`，用户可以在变量文件中完整替换，
例如加入行业 ETF、其他指数或删去不需要的公司。每个清单对象都要完成行情检查和新闻检查，
但不要求每天都有可写内容：
...
中概股、A50、USD/CNH、黄金和原油；宏观的美联储、非农、中国财政部、中国人民银行、
主要央行、机构/投行多空观点和金融大咖言论；国内资金面、债券利率、商品、公司公告和卖方
分歧；以及人工智能、数据中心、芯片、能耗和算力基础设施。每项带有稳定 `id`、显示名称、
