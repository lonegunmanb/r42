...
func TestCollectionRoundsExhausted(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "needs more within limit succeeds",
			maxRounds: intPointer(3),
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.maxRounds != nil && *tt.maxRounds == 2 {
				require.Error(t, err)
...
func TestDefaultMaxCollectionRounds(t *testing.T) {
	t.Parallel()

	state := New(Config{})
	require.NoError(t, state.Begin())
	for range DefaultMaxCollectionRounds - 1 {
		require.NoError(t, state.Advance(EventCollectionCheckpoint))
		require.NoError(t, state.Advance(EventNeedsMore))
	}
	require.NoError(t, state.Advance(EventCollectionCheckpoint))
	assert.Equal(t, DefaultMaxCollectionRounds, state.CollectionRoundsUsed())
	err := state.Advance(EventNeedsMore)
	var budgetErr *BudgetExhaustedError
	require.ErrorAs(t, err, &budgetErr)
}
...
func TestFinalQCReturnPaths(t *testing.T) {
	...
	t.Run("revise research returns to research", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("final qc cannot reopen collection", func(t *testing.T) {
		t.Parallel()
		...
		require.NoError(t, state.Advance(EventSufficient))
		require.NoError(t, state.Advance(EventResearchComplete))
		err := state.Advance(Event("reopen_collection"))
		require.Error(t, err)
		var transitionErr *TransitionError
		require.ErrorAs(t, err, &transitionErr)
		assert.Equal(t, PhaseFinalQC, transitionErr.From)
...
func TestMaxCollectionRoundsReturnsDefensiveCopy(t *testing.T) {
	...
	*first = 99
	...
	assert.Equal(t, DefaultMaxCollectionRounds, *New(Config{}).MaxCollectionRounds())
}
...
func TestInformationNeedOutcomesRoundTrip(t *testing.T) {
	t.Parallel()

	state := New(Config{})
	require.NoError(t, state.Begin())
	assert.Nil(t, state.InformationNeedOutcomes())

	state.SetInformationNeedOutcomes([]byte(`[{"information_need_id":"NEED-001","resolution":"satisfied"}]`))
	assert.JSONEq(t, `[{"information_need_id":"NEED-001","resolution":"satisfied"}]`, string(state.InformationNeedOutcomes()))
