...
- 每完成一个任务，必须先在对应任务表中更新该任务的状态，再开始处理下一个任务。
- 更新任务状态后，必须检查仓库根目录下是否存在名为 `break` 的文件：
  - 如果存在，立即暂停开发，等待人类处理；不得继续执行下一个任务。
  - 如果不存在，才可以继续下一个任务。
- `break` 文件仅作为人工暂停信号，不得删除、修改或绕过。
- 恢复开发前，必须重新检查仓库根目录下的 `break` 文件是否仍然存在。
